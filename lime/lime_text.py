...
class IndexedString(object):
    ...
    def __init__(self, raw_string, split_expression=r'\W+', bow=True,
        ...
        self.as_list = [[s for s in splitter.split(self.raw[i]) if s and s!=' '] for i in range(len(self.raw)) ]
        non_word = splitter.match

        self.max_length = max(len(inner_lst) for inner_lst in self.as_list)
        self.padded_lst = [inner_lst + ['zero'] * (self.max_length - len(inner_lst)) for inner_lst in self.as_list]


        self.as_np = np.array(self.padded_lst)

        self.string_start = np.hstack(
            ([0], np.cumsum([len(x) for x in self.as_np[:-1]]))) # keep track of the starting indices of strings when they are concatenated together.
        ...
        vocab = {} #creates full vocab
        self.inverse_vocab = [] #creates fully  word_idx -> word
        self.positions = []   # word_idx -> (doc, pos)
        self.bow = bow #true
        non_vocab = set()
        self.ndocs = len(self.as_np)

        for k in range(self.ndocs):
            for i, word in enumerate(self.as_np[k]):
                if word in non_vocab:
                    continue
                
                #logging.info(str(k)+" "+str(i)+" "+word)
                if non_word(word):
                    non_vocab.add(word)
                    continue
                ...
                idx_word = vocab[word]
                self.positions[idx_word].append((k,i))

        
        #logging.info(vocab)    
        #logging.info(self.inverse_vocab)
    ...
    #need changes potentially, here and below
    ...
    def inverse_removing(self, words_to_remove):
        ...
        mask = np.ones((self.as_np.shape), dtype='bool') #changed dimension of mask array
        list_of_position_tuples = self.__get_idxs(words_to_remove) 
        #logging.info(list_of_position_tuples)
        for t in list_of_position_tuples: 
            mask[t[0],t[1]] = False #change this line potentially

        #logging.info(mask.nonzero())
        #logging.info(mask.nonzero()[0])
        ...
        for i in range(self.ndocs):
            p = ' '.join([self.as_list[i][v] for v in range(len(self.as_list[i])) if mask[i,v]])
            rem_list.append(p)  

        #logging.info(rem_list)
        return rem_list #return list of strings
    ...
    def __get_idxs(self, words):
        ...
        if self.bow:
            return list(itertools.chain.from_iterable(
                [self.positions[z] for z in words]))   #return will be a list of tuples
        else:
...
class LimeTextExplainer(object):
    ...
    def explain_instance(self,
        ...
        indexed_string = IndexedString(text_instance, bow=self.bow,
                                        ...
                                        mask_string=self.mask_string) #class object
    ...
    def __generate_ordering(self,classifier_fn, query,documents):
        def order_func(s):
            return sorted(range(len(s)), key=lambda k: -s[k])
        labels = []
        for k in range(self.num_samples):
            passages = documents[k]
            scores = classifier_fn.predict([(query,passages[i]) for i in range(len(passages)) ])
            labels.append(order_func(scores))
    ...
    def __data_labels_distances(self,
        ...
        doc_size = indexed_string.num_words() #nvocab
        sample = self.random_state.randint(1, doc_size + 1, num_samples - 1)   # nvocab * 5000
        data = np.ones((num_samples, doc_size)) #(5000, nvocab) all ones
        data[0] = np.ones(doc_size) #first row all ones. entire instance
        features_range = range(doc_size) #(1:nvocab)
        inverse_data = [indexed_string.raw_string()] #list of lists here. original input
        for i, size in enumerate(sample, start=1):
            inactive = self.random_state.choice(features_range, size,
                                                replace=False)
            data[i, inactive] = 0
            inverse_data.append(indexed_string.inverse_removing(inactive)) #change this func
            #logging.info('PASS '+str(i))

        labels = self.__generate_ordering(classifier_fn,query,inverse_data) #output would be an ordering #might need some preprocessing to get labels
